...
m = models.get("Li_DOX_PBPK_mouse.csl")
...
for it in range(1, nits):
    ...
    #m.cl_li   = math.log(random.lognormvariate(585, cv*585))  if using 585, result would be outbond so I didn't use it 
    m.ps_pa   = math.log(random.lognormvariate(8.6, cv*8.6))
    m.ps_sp   = math.log(random.lognormvariate(2.98, cv*2.98))
    m.ps_ad   = math.log(random.lognormvariate(3.79, cv*3.79))
...
plot.xlabel(h, "Time (h)")
plot.ylabel(h, "Liver Concentration  (ug/mL)")
